...
public class StudentQuizResultController {
    ...
    public void setResult(Quiz quiz, double score, double totalPossibleScore, int answeredQuestions, int totalQuestions, boolean timeExpired) {
        ...
        quizTitleLabel.setText(quiz != null ? "Resultat du quiz : " + safeValue(quiz.getTitre()) : "Resultat du quiz");
        ...
        answeredLabel.setText("Questions repondues : " + answeredQuestions + " / " + totalQuestions);
    ...
    private void handleDownloadPdf(ActionEvent event) {
        if (currentQuiz == null) {
            ControllerUtils.showWarning("Aucun resultat a exporter.");
            return;
        }

        FileChooser fileChooser = new FileChooser();
        fileChooser.setTitle("Enregistrer le resultat en PDF");
        ...
        try {
            ...
            ControllerUtils.showInfo("Le resultat a ete enregistre en PDF avec succes.");
        } catch (IOException e) {
            ControllerUtils.showError("La generation du PDF a echoue : " + e.getMessage());
        }
    }
    ...
    private float drawHeader(PDPageContentStream contentStream, PDImageXObject logo, float pageWidth, float topY) throws IOException {
        ...
        writeText(contentStream, "Resultat officiel du quiz", textX, textY - 26f, FONT_REGULAR, 14, 222, 233, 242);
        writeText(contentStream, "Document genere automatiquement pour l'espace etudiant", textX, textY - 46f, FONT_REGULAR, 11, 187, 204, 219);
    ...
    private float drawScoreCard(PDPageContentStream contentStream, float x, float topY, float width) throws IOException {
        ...
        writeText(contentStream, "Questions repondues", rightX, y + 62f, FONT_BOLD, 12, 123, 138, 153);
    ...
    private float drawDetailsSection(PDPageContentStream contentStream, float x, float topY, float width) throws IOException {
        ...
        writeText(contentStream, "Synthese", x + 24f, y + sectionHeight - 28f, FONT_BOLD, 16, 29, 59, 83);
        ...
        String[][] rows = {
                ...
                {"Questions repondues", currentAnsweredQuestions + " / " + currentTotalQuestions},
                {"Duree", formatDuration()},
                {"Niveau", formatLevel()},
                {"Genere le", LocalDateTime.now().format(PDF_DATE_FORMATTER)}
        };
    ...
    private void drawFooter(PDPageContentStream contentStream, float x, float y, float width) throws IOException {
        ...
        writeText(contentStream, "EduVerse - Resultat officiel genere automatiquement", x, y, FONT_REGULAR, 10, 123, 138, 153);
        writeText(contentStream, "Document pret a etre enregistre ou partage", x + width - 190f, y, FONT_REGULAR, 10, 123, 138, 153);
    }
    ...
    private String buildSummaryText() {
        if (currentTimeExpired) {
            return "Temps ecoule. Le quiz a ete soumis automatiquement a la fin du chronometre.";
        }
        if (currentScore > 0) {
            return "Quiz soumis avec succes. Le resultat ci-dessus resume la performance finale de l'etudiant.";
        }
        return "Quiz soumis. Le resultat indique un score nul ou aucune reponse correcte.";
    }
