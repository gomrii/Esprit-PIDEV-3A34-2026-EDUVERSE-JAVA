...
public class TeacherQuizListController {
    ...
    public void chargerQuiz() {
        try {
            masterData = FXCollections.observableArrayList(
                    quizService.afficherQuiz().stream()
                            .filter(ControllerUtils::isQuizOwnedByCurrentUser)
                            .toList()
            );
            applyFiltersAndSort();
    ...
    private void handleOpenStatistics(ActionEvent event) {
        try {
            Parent root = ControllerUtils.loadFxml("/teacher_stats.fxml");
            switchScene(event, root);
    ...
    private void openQuizEditor(Quiz quiz, ActionEvent event) {
        if (ControllerUtils.isAdminOwnedQuiz(quiz)) {
    ...
    private VBox createQuizCard(Quiz quiz) {
        ...
        boolean adminOwned = ControllerUtils.isAdminOwnedQuiz(quiz);
