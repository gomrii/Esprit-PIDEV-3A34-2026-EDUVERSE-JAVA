...
public class ListeReponseController {
    ...
    public void chargerReponses() {
        try {
            ...
            ControllerUtils.showError("Erreur lors du chargement des rÃ©ponses : " + e.getMessage());
        }
    }
    ...
    private void handleEdit(ActionEvent event) {
        if (getSelectedReponse() == null) {
            ControllerUtils.showWarning("SÃ©lectionnez une rÃ©ponse Ã  modifier.");
            return;
        }
    ...
    private void supprimerReponse(ActionEvent event) {
        if (getSelectedReponse() == null) {
            ControllerUtils.showWarning("SÃ©lectionnez une rÃ©ponse Ã  supprimer.");
            return;
        }
        Alert confirm = new Alert(Alert.AlertType.CONFIRMATION, "Supprimer la rÃ©ponse sÃ©lectionnÃ©e ?", javafx.scene.control.ButtonType.YES, javafx.scene.control.ButtonType.NO);
        confirm.showAndWait();
        if (confirm.getResult() == javafx.scene.control.ButtonType.YES) {
            try {
                reponseService.supprimerReponse(getSelectedReponse().getIdReponse());
                ControllerUtils.showInfo("RÃ©ponse supprimÃ©e.");
                chargerReponses();
            } catch (SQLException e) {
    ...
    private void handleBack(ActionEvent event) {
        try {
            ...
            ControllerUtils.showError("Impossible de retourner Ã  l'accueil: " + e.getMessage());
        }
    }
    ...
    private void applyFiltersAndSort() {
        ...
        } else if ("Score dÃ©croissant".equals(sort)) {
